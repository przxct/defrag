...
def correct(sample,gcCount,binSize,maxN=0.1,minRD=0.0001,fVal=0.1,iVal=3):
    ...
    # Ensure values are real and not complex
    allX = np.real(np.array(allX, np.cfloat))
    allY = np.real(np.array(allY, np.cfloat))
    
    if len(allX) == 0 or len(allY) == 0:
        print("Warning: Empty data detected. Returning empty corrected sample.")
        return dict()

    lowessCurve = lowess(allY, allX, frac=fVal, it=iVal).tolist()
    ...
    for chrom in chroms:
        ...
        for bin in range(min(len(gcCount[chrom]),len(sample[chrom]))):
            if gcCount['N'+chrom][bin] < binSize * maxN and sample[chrom][bin] > binSize * minRD:
                if lowessCurve:  # Ensure lowessCurve is not empty
                    correctedValue = sample[chrom][bin]/lowessCurve.pop(0)[1]
